...
export default function ProjectsPage() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      // Only fetch non-archived projects
      const { data, error } = await supabase
        .from('projects')
        .select('*')
        .eq('archived', false)  // Only show non-archived projects
  ...
  const filteredProjects = projects.filter(project => {
    ...
    // Status filter - make sure we're comparing correctly
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {statusOptions.filter(s => s !== 'all' && s !== 'archived').map(status => (
          ...
          {(searchTerm || filterPriority !== 'all' || filterStatus !== 'all') && (
            <button
              ...
              className="px-3 py-2 border border-border rounded-lg hover:bg-muted transition-colors flex items-center gap-1"
            >
          ...
          {filteredProjects.map((project) => (
            ...
            >
              ...
              {project.tech_stack && project.tech_stack.length > 0 && (
                <div className="flex flex-wrap text-gray-500 dark:text-gray-700 gap-1 mb-4">
                  {project.tech_stack.slice(0, 3).map((tech) => (
                    <span
                      key={tech}
                      className="px-2 py-0.5 bg-blue-100 text-xs rounded-full"
                    >
                      {tech}
                    </span>
                  ))}
